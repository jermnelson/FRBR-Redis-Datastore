...
FRBR_ and extending using object-orientated FRBRoo_ enabling support for both
...
class AggregationWork(Work):
    ...
    def get_members(self):
        return work_members.objects.filter(work=self)

    def save(self):
        child_members = work_members.objects.filter(work=self)
        if child_members:
            for member in child_members:
                if not isinstance(member,ContainerWork) or\
                   not isinstance(member,IndividualWork):
                    raise ValueError("All member Works in an AggregationWork "\
...
class SerialWork(ComplexWork,PublicationWork):
    ...
    has_issuing_rule = generic.GenericForeignKey('content-type','object_id') 
